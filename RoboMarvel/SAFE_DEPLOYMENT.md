...
```bash
export ROBOT_PASSWORD='ввести локально, не сохранять в Git'
export YANDEX_ENV_PATH=/tmp/fleet-yandex-cloud.env
./deploy-fleet.sh 36 99 92 83
```

Скрипт сначала проверяет канонический bind-mount `/home/robomarvel/robomarvel`,
ROS Jazzy, синхронизацию времени, image ID, Compose и Wi-Fi профиль. Только после
этого он передаёт секретный env. При ошибке следующая группа не запускается;
подробные логи остаются в `fleet_logs/`. Нестандартный студенческий проект нельзя
форсировать: сначала согласовать миграцию и сохранить его контейнеры для отката.
